...
class NSurv_MMD(Model):
    ...
    def __init__(self,
        ...
        self.decoder_layers = [self.dense(ls) for ls in decoder_layer_sizes] + [Dense(n_bins + 1, activation='softmax')]
    ...
    def loss(self, x, y, s, mmd_binary_variable):
        
        nll = tf.reduce_mean(self.nll(x, y, s))
        l = nll + tf.reduce_sum(self.losses)
        
        # MMD Term
        l += self.ld * tf.cast(self.mmd(self.representation, mmd_binary_variable), dtype=tf.float32)
    ...
    def nll(self, x, y, s):
        ...
        Ft = tf.reduce_sum(yt * self._survival_from_density(t_pred)[:, :-1], axis=1) + self.tol
...
def train_model_MMD(
    ...
    #@tf.function
    def train_step(x, y, s, mbv):
        with tf.GradientTape() as tape:
            train_loss, train_nll = model.loss(x, y, s, mbv)
            #print(train_loss, train_nll)
        grads = tape.gradient(train_loss, model.trainable_weights)
        optimizer.apply_gradients(zip(grads, model.trainable_weights))
        return train_loss, train_nll
    ...
    #@tf.function
    def test_step(x, y, s, mbv):
        val_loss, val_nll = model.loss(x, y, s, mbv)
        return val_loss, val_nll
    ...
    for epoch_idx in range(n_epochs):
        ...
        for batch_idx, (xt, yt, st, mbvt) in enumerate(get_batches(*train_data, batch_size=batch_size)):

            train_loss, train_nll = train_step(xt, yt, st, mbvt)
        ...
        # Run a validation loop at the end of each epoch.
        for batch_idx, (xv, yv, sv, mbvv) in enumerate(get_batches(*val_data, batch_size=batch_size)):

            val_loss, val_nll = test_step(xv, yv, sv, mbvv)
